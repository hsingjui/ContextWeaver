...
```
Phase 1: Plan    → plan only when the user explicitly asks to create a task, then write planning artifacts
...
- **Do NOT create a Trellis task unless the user explicitly says to create one.** Handle requests directly in the main session.
...
No active task. Do NOT create a Trellis task unless the user explicitly asks for one. Handle the request directly in the main session.
[/workflow-state:no_task]
...
### Phase 1: Plan
- 1.0 Create task `[required · once]` (only when the user explicitly asks to create one)
- 1.1 Requirement exploration `[required · repeatable]` (`prd.md`; complex tasks also need `design.md` + `implement.md`)
- 1.2 Research `[optional · repeatable]`
- 1.3 Configure context `[required · once]` — skip: sub-agent dispatch is disabled, context loads via the `trellis-before-dev` skill
...
Sub-agent dispatch is **DISABLED**. Never spawn `trellis-implement` / `trellis-check` / `trellis-research` sub-agents on any platform. All implementation, checks, and research happen in the main session.

[workflow-state:in_progress]
Flow: `trellis-before-dev` -> edit -> `trellis-check` (skill) -> validation -> `trellis-update-spec` -> commit (Phase 3.4) -> `/trellis:finish-work`.
Sub-agent dispatch is DISABLED — never spawn `trellis-implement` / `trellis-check` / `trellis-research`; do the work in the main session.
...
Goal: create a task and produce planning artifacts only when the user explicitly asks to create a task; otherwise handle the request directly in the main session.

#### 1.0 Create task `[required · once]`

Create the task directory only when the user explicitly asked to create a task. The command sets status to `planning`, writes `task.json`, creates a default `prd.md`, and auto-targets the new task when session identity is available:
...
Research happens in the main session directly. Write findings into `{TASK_DIR}/research/`. No sub-agents.
...
Sub-agent dispatch is DISABLED — implement in the main session:
...
Sub-agent dispatch is DISABLED — check in the main session:
...
- No active task must NOT create a Trellis task unless the user explicitly asks for one.
