...
import { getVectorStore, type VectorStore, type ChunkRecord } from "../vectorStore/index.js";
import { getEmbeddingConfig } from "../config.js";
import { logger } from "../utils/logger.js";
import type { SearchConfig, ScoredChunk } from "./types.js";
...
export class GraphExpander {
    ...
    private async expandImports(
        ...
        while (queue.length > 0) {
            ...
            // 2. 获取文件内容
            const row = this.db!.prepare("SELECT content FROM files WHERE path = ?").get(filePath) as { content: string } | undefined;
...
/**
 * 使指定项目的 GraphExpander 文件索引失效
 */
export function invalidateGraphExpanderFileIndex(projectId: string): void {
