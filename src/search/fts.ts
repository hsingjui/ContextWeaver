 ...
 */
export function detectFtsTokenizer(db: Database.Database): FtsTokenizer {
    // 检查缓存
 ...
 * 检查两表记录数差异，必要时重建索引
 */
export function syncFilesFts(db: Database.Database): void {
    const fileCount = (db.prepare("SELECT COUNT(*) as c FROM files WHERE content IS NOT NULL").get() as { c: number }).c;
...
export function upsertChunkFts(
    ...
    ).run(chunkId, filePath, chunkIndex, breadcrumb, content);
}
...
/**
 * 删除文件的所有 chunk FTS 索引
 */
export function deleteFileChunksFts(db: Database.Database, filePath: string): void {
    db.prepare("DELETE FROM chunks_fts WHERE file_path = ?").run(filePath);
}
 ...
 */
export function searchChunksFts(db: Database.Database, query: string, limit: number): ChunkFtsResult[] {
    // 使用统一分词器
 ...
 * 插入或更新 FTS 索引
 */
export function upsertFileFts(db: Database.Database, path: string, content: string): void {
    // FTS5 不支持 ON CONFLICT，需要先删后插
 ...
 * 批量更新 FTS 索引
 */
export function batchUpsertFileFts(db: Database.Database, files: Array<{ path: string; content: string }>): void {
 ...
 * 删除 FTS 索引记录
 */
export function deleteFileFts(db: Database.Database, path: string): void {
    db.prepare("DELETE FROM files_fts WHERE path = ?").run(path);
}
 ...
 */
export function searchFilesFts(db: Database.Database, query: string, limit: number): FtsSearchResult[] {
    // 1. 使用统一分词器
    const tokens = segmentQuery(query);
