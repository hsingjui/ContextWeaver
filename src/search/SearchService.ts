...
import { searchFilesFts, searchChunksFts, isFtsInitialized, isChunksFtsInitialized, segmentQuery } from "./fts.js";
...
import type { SearchConfig, ScoredChunk, ContextPack } from "./types.js";
...
export class SearchService {
    ...
    constructor(projectId: string, projectPath: string, config?: Partial<SearchConfig>) {
    ...
    private extractAroundHit(text: string, queryTokens: Set<string>, maxLen: number): string {
        ...
        while (currentLen < maxLen) {
            ...
            // 如果两边都无法扩展了，退出
            if ((start === 0 || lines[start - 1].length + 1 + currentLen > maxLen) &&
